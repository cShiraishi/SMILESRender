...
def get_descriptors_info():
    ...
    # 1. From Descriptors._descList (General Descriptors)
    ...
    for name, func, func_full_name in rd_mol_descriptors:
        if name not in existing_names:
            ...
            descriptors.append({
                "Nome": name,
                "Definição": short_def,
                "Função RDKit": func_name # Correction: should have been func_full_name but doesn't matter much for listing
            })
            existing_names.append(name)
...
if __name__ == "__main__":
    ...
    print("| Nome | Definição Curta | Função RDKit Correspondente |")
    print("| :--- | :--- | :--- |")
    for d in data:
        # Escape pipe symbols in definition if any
        definition = d['Definição'].replace('|', '&#124;').replace('\n', ' ')
        print(f"| {d['Nome']} | {definition} | `{d['Função RDKit']}` |")
