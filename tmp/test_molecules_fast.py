...
try:
    ...
    result = opener.open(req_post, timeout=5).read().decode('utf-8')
    print("ADMETLab3 SUCCESS", flush=True)
except Exception as e:
    print(f"ADMETlab failed: {e}", flush=True)

print("TESTING pkCSM...", flush=True)
try:
    ...
    result2_url = urllib.request.urlopen(req_pkcsm, timeout=5).geturl()
    print("pkCSM URL:", result2_url, flush=True)
except Exception as e:
    print(f"pkCSM failed: {e}", flush=True)
